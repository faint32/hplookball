...
public class AsyncHttpClient extends BaseHttpClient {
	...
	private static final int DEFAULT_SOCKET_BUFFER_SIZE = 1024*1024;
...
//							HupuLog.d("ResponseInterceptor", "gzip");
				...
				if (headers != null) {
					for (int i = 0; i < headers.length; i++) {
						if (headers[i].getName().equalsIgnoreCase(HEADER_DATE)) {
							Date date = new java.util.Date(headers[i]
									.getValue());
							today = date.getTime();
	...
	/**
	 * update the User-Agent header to be sent with each request.
	 * 
	 * @param nt
	 *            network type of mobile,-1 means wifi
	 * 
	 */
	...
	public void get(Context context, String url, RequestParams params,
		...
		if (url == null)
			((AsyncHttpResponseHandler)responseHandler).sendFailureMessage(new Throwable("请求地址不完整"),
					"请求地址不完整", ((AsyncHttpResponseHandler)responseHandler).reqCode);
		else
			sendRequest(httpClient, httpContext, new HttpGet(
					getUrlWithQueryString(url, params)), null, (AsyncHttpResponseHandler)responseHandler,
					context);
	}
	...
	public void get(Context context, String url, Header[] headers,
		...
		if (url == null)
			responseHandler.sendFailureMessage(new Throwable("请求地址不完整"),
					"请求地址不完整", responseHandler.reqCode);
		else {
	...
	public void post(Context context, String url, RequestParams params,
			BaseHttpResponseHandler responseHandler) {
		post(context, url, paramsToEntity(params), null, (AsyncHttpResponseHandler)responseHandler);
	}
	...
	protected void sendRequest(DefaultHttpClient client,
		...
		Future<?> request = threadPool
				.submit(new AsyncHttpRequest(client, httpContext, uriRequest,
						responseHandler, responseHandler.reqCode));
