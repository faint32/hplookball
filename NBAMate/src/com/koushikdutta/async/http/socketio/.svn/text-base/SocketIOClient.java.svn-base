...
public class SocketIOClient extends EventEmitter {
    ...
    public void emitJsonObj(String name, Object... args) {
        emit(name, new JSONArray(Arrays.asList(args)), null);
    }
...
//        final SocketIOConnection connection = new SocketIOConnection(handler, client, request);
        if(mConnection ==null)
        {
        	mConnection= new SocketIOConnection(handler, client, request);
        }
        else
        	mConnection.setRequest(request);
        final ConnectCallback wrappedCallback = new ConnectCallback() {
            ...
            public void onConnectCompleted(Exception ex, SocketIOClient client) {
                ...
                mConnection.clients.remove(client);
        ...
        mConnection.clients.add(new SocketIOClient(mConnection, "", wrappedCallback));
        mConnection.reconnect(ret);
